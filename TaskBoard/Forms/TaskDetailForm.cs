    ...
    public partial class TaskDetailForm : Form
    {
        ...
        private void btnAddFile_Click(object? sender, EventArgs e)
        {
            ...
            foreach (var filePath in ofd.FileNames)
            {
                ...
                {
                    // Programı çökertmeden kullanıcıya bilgi ver
                    MessageBox.Show($"'{Path.GetFileName(filePath)}' dosyası eklenirken bir hata oluştu:\n{ex.Message}",
                        "Dosya Ekleme Hatası", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            LoadAttachments();
        }
        ...
        private void btnDeleteFile_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                // Önce fiziksel dosyayı diskten sil
                if (File.Exists(att.FilePath))
                {
                    File.Delete(att.FilePath);
                }

                // Eğer diskten silindiyse veritabanından kaydını uçur
                using var db = new AppDbContext();
                var dbAtt = db.Attachments.Find(att.Id);
                if (dbAtt != null)
                {
                    db.Attachments.Remove(dbAtt);
                    db.SaveChanges();
                }
            ...
            {
                MessageBox.Show("Bu dosya şu anda başka bir program tarafından kullanıldığı için silinemiyor. Lütfen arka planda dosyayı kapatıp tekrar deneyin.",
                    "Dosya Kullanımda", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Dosya silinirken beklenmedik bir hata oluştu:\n{ex.Message}",
